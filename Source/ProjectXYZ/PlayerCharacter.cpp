...
void APlayerCharacter::DisposeSpellRef()
{
...
{
	if(currentSpell.IsValid())
	currentSpell->EndBehaviorLowLevel(); 
}
...
void APlayerCharacter::ReleaseSpellForward()
{
    ...
    if (waterIndex != INDEX_NONE)
    {
        ...
        {
            int32 coldIndex = elementQueue.Find(ElementID::Cold);
            if (coldIndex != INDEX_NONE)
            {
                elementQueue.RemoveAtSwap(coldIndex);
...
///*Run only on server, propagate to clients*/
///*Role_Authority*/
//void APlayerCharacter::BeingPushed(const FVector& amount)
//{
//	this->State = BUSY_PUSHED;
//	this->AddActorWorldOffset(amount, true);
//	// Change animation etc;
...
//		currentSpell = GetWorld()->GetGameState<ACustomGameState>()->genSpell(serverSideElementQueue, false);
//	}
//	currentSpell->StartBehaviorLowLevel(*this);
//}
